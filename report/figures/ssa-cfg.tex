...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm]
    ...
    \node[vis/semantic node,minimum width=3.75cm] (loop) at (0,0)
      {\texttt{loop}\quad $i=\phi(0,i')$\\$acc=\phi(0,acc')$\quad $i<k$?};
    \node[vis/artifact,text width=3.55cm] (body) at (-1.15,-1.85)
      {\texttt{body}\quad load--multiply--clip\\$i'=i+1,\quad acc'=acc+term$};
    \node[vis/artifact,minimum width=2.25cm] (exit) at (1.9,-1.85)
    ...
    \draw[vis/deferred edge] (cfg.east) -- node[vis/label,above]{降低时对应} (corr.west);
  \end{tikzpicture}
