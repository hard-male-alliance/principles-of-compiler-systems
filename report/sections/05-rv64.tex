...
LP64D 调用约定以 \code{a0--a7} 传递整数参数，以 \code{a0}/\code{a1} 承载至多两个寄存器返回值；本例的单个 \code{int} 返回值只使用 \code{a0}。\code{ra} 保存返回地址；\code{sp} 是栈指针并须在过程执行期间保持 16 字节对齐；\code{t0--t6} 为调用者保存临时寄存器；\code{s0--s11} 为被调用者保存\cite{riscv-psabi-1.0}。因此 \code{factorial} 从 \code{a0} 读入 $n$，在 \code{t0}/\code{t1} 中维护循环状态，再把结果移回 \code{a0}。
